...
namespace StevEngine::Physics {
	...
	Vector3d CubeCollider::CollisionPoint(Collider* other) {
		...
		double maxX = std::fmin(ownRange.High.X, otherRange.High.X);
		double minX = std::fmax(ownRange.Low.X, otherRange.Low.X);
		double maxY = std::fmin(ownRange.High.Y, otherRange.High.Y);
		double minY = std::fmax(ownRange.Low.Y, otherRange.Low.Y);
		double maxZ = std::fmin(ownRange.High.Z, otherRange.High.Z);
		double minZ = std::fmax(ownRange.Low.Z, otherRange.Low.Z);
