...
void SpawnPhysicsObjects(Scene& scene) {
	GameObject& object = scene.CreateObject("Floor", Vector3(0), Quaternion(), Vector3(AREA_SIZE, 1, AREA_SIZE));
	...
	for(int i = 0; i < PHYSICS_OBJECTS; i++) {
		GameObject& object = scene.CreateObject("Cube" + std::to_string(i), Vector3(GetRandomDouble(-AREA_SIZE / 2.0, AREA_SIZE / 2.0), 10, GetRandomDouble(-AREA_SIZE / 2.0, AREA_SIZE / 2.0)), Quaternion(), Vector3(1, 1, 1));
		#ifdef StevEngine_SHOW_WINDOW
		Visuals::CubePrimitive* primitive = object.AddComponent(new Visuals::CubePrimitive());
	...
	for(int i = 0; i < AREA_SIZE; i++) {
		GameObject& object = scene.CreateObject("Light" + std::to_string(i), Vector3(GetRandomDouble(-AREA_SIZE / 2.0, AREA_SIZE / 2.0), 15, GetRandomDouble(-AREA_SIZE / 2.0, AREA_SIZE / 2.0)), Quaternion(), Vector3(1, 1, 1));
