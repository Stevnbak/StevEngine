...
cd $root/JoltPhysics
cd Build
cmake -S . -B Linux_Debug -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER=$COMPILER "${@}" -DGENERATE_DEBUG_SYMBOLS=OFF -DDEBUG_RENDERER_IN_DEBUG_AND_RELEASE=OFF -DPROFILER_IN_DEBUG_AND_RELEASE=OFF -DUSE_STD_VECTOR=OFF -DDOUBLE_PRECISION=OFF -DUSE_ASSERTS=OFF -DENABLE_OBJECT_STREAM=OFF
